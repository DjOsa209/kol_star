...
export function useRole(treeRef: Ref) {
  ...
  function onChange({ row, index }) {
    ElMessageBox.confirm(
      ...
      .then(() => {
        ...
        updateRoleStatus({ id: row.id, status: row.status }).then(() => {
          switchLoadMap.value[index] = Object.assign(
            {},
            switchLoadMap.value[index],
            {
  ...
  function handleDelete(row) {
    ElMessageBox.confirm(`确认要删除角色 ${row.name} 吗?`, "系统提示", {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
      draggable: true
      ...
      if (code === 0) {
        message(`已删除角色名称为${row.name}的这条数据`, {
          type: "success"
        });
        onSearch();
      }
    ...
    addDialog({
      ...
      beforeSure: (done, { options }) => {
        ...
        FormRef.validate(async valid => {
          if (valid) {
            const action = title === "新增" ? createRole : updateRole;
            const { code } = await action(curData);
            if (code !== 0) return;
            if (title === "新增") {
